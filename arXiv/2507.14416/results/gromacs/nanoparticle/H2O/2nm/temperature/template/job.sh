...
# Add solvate
$GMX_CMD editconf -f ./gro/input.gro -o ./gro/input_box.gro -c -d 0.5 -bt cubic

$GMX_CMD insert-molecules -f ./gro/input_box.gro -ci ./gro/tip4p.gro -o ./gro/input_w_water.gro -try 5000 -nmol $H2O_MOLECULES -scale 1.0
echo q | $GMX_CMD make_ndx -f gro/input_w_water.gro -o ndx/index.ndx
cp top/init_topol.top top/topol.top
WATER_MOLECULES=$(( $(grep -c SOL ./gro/input_w_water.gro) / 4 ))
...
#Energy minimization
(
$GMX_CMD grompp -f ./mdp/min.mdp -c ./gro/input_w_water.gro -r ./gro/input_w_water.gro -p ./top/topol.top -o ./min/input.tpr -po ./min/mdout.mdp -n ./ndx/index.ndx -maxwarn $MAXWARN
...
for temperature in 400 350 300 250 200 150 100
do
  rm -rf npt_${temperature}
  mkdir npt_${temperature}
  cp mdp/npt.mdp npt_${temperature}/input.mdp
  (
    sed -i "/annealing-temp/c annealing-temp = ${init_temperature} ${temperature} ${init_temperature} ${temperature}" npt_${temperature}/input.mdp
    sed -i "/ref-t/c ref-t = ${temperature} ${temperature}" npt_${temperature}/input.mdp
  ...
  (
    sed -i "/ref-t/c ref-t = ${temperature} ${temperature}" nvt_${temperature}/input.mdp
  ...
  (
    sed -i "/ref-t/c ref-t = ${temperature} ${temperature}" md_prod_${temperature}/input.mdp
  ...
  (
    sed -i "/ref-t/c ref-t = ${temperature} ${temperature}" md_phonon_${temperature}/input.mdp
