...
def convert(runset,temperature):
    ...
    parameters['output_file'] = (u'%s/nvt_phonon_%dK' % (runset,temperature), 'netcdf')
    parameters['smart_mass_association'] = True
    parameters['time_step'] = 1.0
    parameters['trajectory_file'] = u'../nvt_phonon_%s_%dK.lammpstrj' % (runset,temperature)
...
def dos(temperature,weights,runset,groupping):
    parameters = {}
    ...
    parameters['output_files'] = (u'%s/%s_%s_%dK_dos'%(runset,groupping,weights,temperature), (u'hdf',))
    parameters['projection'] = None
    parameters['running_mode'] = ('monoprocessor',)
    parameters['trajectory'] = u'%s/nvt_phonon_%dK.nc'%(runset,temperature)
    ...
    print("processing dos at %dK - %s %s %s"%(temperature,groupping,weights,runset))
...
for temperature in [200, 300]:
    for runset in ["total","core","shell"]:
        convert(runset,temperature)
        for groupping in [u'atom',u'group']:
            dos(temperature,"b_incoherent",runset,groupping)
